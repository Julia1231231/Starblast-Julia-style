...
(function () {
  ...
  function initSocket(ws){
    ...
    ws.addEventListener('message',(ev)=>{ if(!(ev.data instanceof Blob)) return;
      ev.data.arrayBuffer().then((ab)=>{ const u8=new Uint8Array(ab); if(u8[0]!==240) return;
        ...
        else{ const text=decodeTransport(next); const who=(ownId!=null && senderId===ownId)?'You':(playerInfo[senderId]?.name||`ID${senderId}`); const hue=(ownId!=null && senderId===ownId)?310:playerInfo[senderId]?.hue;
