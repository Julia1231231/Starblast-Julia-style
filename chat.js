(function (root, factory) {
  ...
  function b64urlEncodeBytes(u8){ let s=''; for(let i=0;i<u8.length;i++) s+=String.fromCharCode(u8[i]); return btoa(s).replace(/\+/g,'-').replace(/\//g,'_').replace(/=+$/,''); }
  function b64urlDecodeToBytes(b64){ let s=b64.replace(/-/g,'+').replace(/_/g,'/'); const pad=s.length%4?4-(s.length%4):0; s+='='.repeat(pad); const bin=atob(s); const out=new Uint8Array(bin.length); for(let i=0;i<bin.length;i++) out[i]=bin.charCodeAt(i); return out; }

  function encodeTransport(str){
    const t=String(str||'');
    if (/^[A-Za-z0-9 ]*$/.test(t)) return t.replace(/ /g,'+');
    return b64urlEncodeBytes(utf8Encode(t));
  }
  function decodeTransport(enc,isPlain){
    if (isPlain) return enc.replace(/\+/g,' ');
    return utf8Decode(b64urlDecodeToBytes(String(enc||'')));
  }
