(function (root, factory) {
  ...
  const PLAIN_MARK='P:', ENC_MARK='E:';

  function encodeTransport(str){
    const t=String(str||'');
    if (/^[A-Za-z0-9 ]*$/.test(t)) return PLAIN_MARK + t;
    return ENC_MARK + enc62Bytes(utf8Encode(t));
  }
  function decodeTransport(data){
    const s=String(data||'');
    if (s.startsWith(PLAIN_MARK)) return s.slice(2);
    if (s.startsWith(ENC_MARK)) return utf8Decode(dec62ToBytes(s.slice(2)));
    return s;
  }
